#!/usr/bin/env python3
"""
CS 课程学习报告 Markdown → PDF 转换脚本 (WeasyPrint版)
...
def md_to_html(md_text, title="CS 课程学习报告", subtitle="CS Lecture Study Report",
               meta_line="", author="CS Lecture Learn"):
    ...
    if first_h1_match:
        ...
        if not title or title == "CS 课程学习报告":
            title = extracted_title
        html_body = html_body.replace(first_h1_match.group(0), '', 1)

    # 替换 CSS 中的页眉占位符
    css = CSS_TEMPLATE.replace("HEADER_TEXT", f"{title}  |  CS Lecture Study Report")
...
def main():
    parser = argparse.ArgumentParser(description="CS 课程学习报告 Markdown → PDF")
    parser.add_argument("input", help="输入的 Markdown 文件路径")
    parser.add_argument("output", help="输出的 PDF 文件路径")
    parser.add_argument("--title", default=None, help="报告标题")
    parser.add_argument("--author", default="CS Lecture Learn", help="作者名")
    ...
    html = md_to_html(md_text, title=args.title or "CS 课程学习报告", meta_line=meta_line, author=args.author)
